...
function createClient(args, options) {
  ...
  let protoLoaderOptions = (args.includeDirs) ? {
    includeDirs: [args.includeDir]
  } : {};

  let a = protoLoader.loadSync(file.root + '/' + args.proto, protoLoaderOptions);

  let parsed = grpc.loadPackageDefinition(a);
